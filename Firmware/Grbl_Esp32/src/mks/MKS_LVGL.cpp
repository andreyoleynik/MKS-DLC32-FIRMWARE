...
// Буфер частичной перерисовки LVGL: раньше 30 строк экрана (480*30*2 = 28800
// байт статического BSS, ~27% всей занятой RAM прошивки) держались в памяти
// постоянно, независимо от состояния Wi-Fi/Telnet/аплоада. Уменьшение до 10
// строк освобождает ~19 КБ статической RAM для кучи (heap), что напрямую
// снижает фрагментацию/нехватку памяти во время сетевых операций. Экран при
// этом просто чуть чаще вызывает flush() на меньшие области — визуально
// незаметно для HMI-интерфейса станка.
